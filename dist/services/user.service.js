...
let UserService = class UserService {
    ...
    async getUsers(offset = 0, limit = 10) {
        try {
            ...
            if (!users.length) {
                throw new common_1.NotFoundException('Không có người dùng nào trong hệ thống.');
            }
    ...
    async getMeetingDays(userId) {
        try {
            ...
            if (!userMeetings.length) {
                throw new common_1.NotFoundException(`Không có cuộc họp nào cho người dùng với ID ${userId}.`);
            }
            const meetingDays = [];
